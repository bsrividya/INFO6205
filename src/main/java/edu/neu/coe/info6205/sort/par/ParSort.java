...
class ParSort {
    ...
    public static void sort(int[] array, int from, int to, ForkJoinPool threadpool) {
        if (to - from < cutoff) Arrays.sort(array, from, to);
            ...
            CompletableFuture<int[]> parsort1 = parsort(array, from, from + (to - from) / 2, threadpool); // TO IMPLEMENT
            CompletableFuture<int[]> parsort2 = parsort(array, from + (to - from) / 2, to, threadpool); // TO IMPLEMENT
